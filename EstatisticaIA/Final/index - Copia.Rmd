...
Isso pode indicar presença de Outliers. A Conslusão que já se pode tirar é que há erros no Teor alcoolico, haja visto que é sabido que não existe vinhos com teor alcoolico a baixo de 8. 
...
```{r chunk="idx_03_01" , echo=TRUE, eval=TRUE}
  ...
  #removendo os dataframes desnecessarios
  rm("vinhos_ajustado", "vinhos")
...
df_branco <- df_branco[!abs(df_branco$acucar_residual)>3,]
...
ggcorr(df_tinto, method = c("all.obs", "spearman"))
```  
...
cor(df_branco)
cor(subset(vinhos_ajustado[,2:11], tipo!="RED"))

ggcorr(df_branco, method = c("all.obs", "spearman"))
