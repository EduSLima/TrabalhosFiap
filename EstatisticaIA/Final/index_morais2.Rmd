...
vinhos <- read_csv2("C:/Users/Eduardo/Documents/GitHub/TrabalhosFiap/EstatisticaIA/Final/Arquivos/BaseWine_Red_e_White.csv" ,col_names = nome_colunas, skip = 1)
...
### Algumas relações que conseguimos inferir com algortimo K-means para as variáveis alcoólico, sulfatos e pH na base vinho branco e conclusões gerais desse algoritmo
...
Considerações gerais, o autor poderia ter segmentado a variável "Sulphates", em tipos de sulfatos ou tê-la nomeado diferente, pois na análise química precisamos saber qual é o íon do sulfato. Em pesquisas de vinhos feitas é comumente nomeado como "sulfatos" o K2SO4 e sua unidade de medida é em ( g/L ).
Já o SO2 é conhecido como "anidro sulfuroso". Os sulfatos aparecem por oxidação de sulfitos, assim, sua quantificação auxilia a inferir sobre a produção de dióxido de enxofre(SO2). Legislação(Lei nº 7.678) que determina o máximo de 1,00 g/Lde K2SO4, com a visualização fácil através do histograma poderiámos sinalizar para importadora não comprar os vinhos com teor muito altos em ranges acima de 0.9.