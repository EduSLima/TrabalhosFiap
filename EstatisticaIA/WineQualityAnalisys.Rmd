...
Pacotes_Necessarios <- c("ggplot2","readr","dplyr", "corrgram","corrplot", "plotly","skimr","gridExtra","GGally","gmodels","ggpubr")
...
#vinhos$VinhoBranco <- (ifelse(vinhos$tipo =='WHITE',1,0))
#vinhos$VinhoTinto <- (ifelse(vinhos$tipo =='RED',1,0))
...
Exibindo a Correlação do dataset observando apenas os dados referentes ao vinho branco
...
#AUTOMATIZAR ESSA PARTE PARA LER TODAS AS VARIAVEIS 
  ...
  pf <- qplot( qualidade, grau_alcolico,data=vinhos_ajustado)+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pt <- qplot( qualidade, grau_alcolico,data=subset(vinhos_ajustado, tipo=="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pb <- qplot( qualidade, grau_alcolico,data=subset(vinhos_ajustado, tipo!="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))

  
  ggarrange(pf,                                                 
          ggarrange(pt, pb, ncol = 2, labels = c("Tinto", "Branco")), 
          nrow = 2, 
          labels = "Todos os Tipos de Vinho"                                       
          ) 
  
  
  pf <- qplot( qualidade, acidez_fixa,data=vinhos_ajustado)+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pt <- qplot( qualidade, acidez_fixa,data=subset(vinhos_ajustado, tipo=="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pb <- qplot( qualidade, acidez_fixa,data=subset(vinhos_ajustado, tipo!="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  ...
  pf <- qplot( qualidade, acucar_residual,data=vinhos_ajustado)+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pt <- qplot( qualidade, acucar_residual,data=subset(vinhos_ajustado, tipo=="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
  pb <- qplot( qualidade, acucar_residual,data=subset(vinhos_ajustado, tipo!="RED"))+ geom_smooth(method = "glm", formula = y~x, family = gaussian(link = 'log'))
